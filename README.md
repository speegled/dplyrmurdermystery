...
Install this package by typing `remotes::install_github("speegled/dplyrmurdermystery")`. Use ` data("dplyr_murder_mystery")` to load the data. Data is **not** lazy loaded, because the only reason to load the package is if you want to solve the murder mystery. The data is about 1MB.
